...
DetectorConstruction::DetectorConstruction(char * nameofione)
...
*/
  ...
  // Target
  G4ThreeVector positionTarget = G4ThreeVector(0,0,.5 *um);// (0,0,-(targetSize+trackerSize));
...
*/
  ...
  TrackerSD* aTrackerSD = new TrackerSD(trackerSDname, densityMembran / kg * m3, molar, 2*fTargetWidth, nameofion);
...
//--------- User Limits -------------------------------
 // Sets a max Step length in the tracker region, with G4StepLimiter
  G4double maxStep = 1*um; // try to set max step to 10^-9 of world's lenght (about 1 nm)
  stepLimit = new G4UserLimits(maxStep); // doesn't work now 
  //stepLimit->SetUserMinEkine(1*MeV);
